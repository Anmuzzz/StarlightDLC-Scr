...
import com.isusdlc.utility.math.calculator.function.Functions;
import com.isusdlc.utility.math.calculator.operator.Operator;
import com.isusdlc.utility.math.calculator.operator.Operators;
...
public class Tokenizer {
      ...
      if (!Character.isDigit(ch) && ch != '.') {
         if (this.isArgumentSeparator(ch)) {
            ...
            if (this.lastToken != null
               ...
               && this.lastToken.getType() != 2
               && this.lastToken.getType() != 4
               && this.lastToken.getType() != 3
               && this.lastToken.getType() != 7) {
   ...
   private Token parseFunctionOrVariable() {
      ...
      for (int testPos = offset + len - 1;
         !this.isEndOfExpression(testPos) && isVariableOrFunctionCharacter(this.expression[testPos]);
      ...
      if (this.isEndOfExpression(offset + len)) {
         ...
         while (
            !this.isEndOfExpression(offset + len)
               && isNumeric(this.expression[offset + len], this.expression[offset + len - 1] == 'e' || this.expression[offset + len - 1] == 'E')
         ) {
            len++;
            this.pos++;
         }

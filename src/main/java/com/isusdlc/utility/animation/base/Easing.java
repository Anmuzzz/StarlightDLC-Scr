...
public interface Easing {
   ...
   Easing BOTH_CUBIC = (t, b, c, d) -> {
      float x = c * t / d + b;
      return x < 0.5 ? 4.0F * x * x * x : (float)(1.0 - Math.pow(-2.0F * x + 2.0F, 3.0) / 2.0);
   };
   ...
   Easing SINE_IN = (t, b, c, d) -> -c * (float)MathUtility.cos(t / d * (Math.PI / 2)) + c + b;
   Easing SINE_OUT = (t, b, c, d) -> c * (float)MathUtility.sin(t / d * (Math.PI / 2)) + b;
   Easing SINE_IN_OUT = (t, b, c, d) -> -c / 2.0F * ((float)MathUtility.cos(Math.PI * t / d) - 1.0F) + b;
   ...
   static Easing generate(double x1, double y1, double x2, double y2) {
      return new Easing() {
         ...
         private float solveTBez(float x1, float x2, float progress) {
            ...
            for (int i = 0; i < 8; i++) {
               float x = this.bezierX(t, x1, x2);
               float dx = this.bezierDX(t, x1, x2);
   ...
   public static class ElasticIn extends Easing.Elastic {
      ...
      public float ease(float t, float b, float c, float d) {
         ...
         if (t == 0.0F) {
            ...
            return -(a * (float)Math.pow(2.0, 10.0F * --t) * (float)MathUtility.sin((t * d - s) * (Math.PI * 2) / p)) + b;
         }
      }
   }
   ...
   public static class ElasticInOut extends Easing.Elastic {
      ...
      public float ease(float t, float b, float c, float d) {
         ...
         if (t == 0.0F) {
            ...
            return t < 1.0F
               ? -0.5F * (a * (float)Math.pow(2.0, 10.0F * --t) * (float)MathUtility.sin((t * d - s) * (Math.PI * 2) / p)) + b
               : a * (float)Math.pow(2.0, -10.0F * --t) * (float)MathUtility.sin((t * d - s) * (Math.PI * 2) / p) * 0.5F + c + b;
         }
      }
   }
   ...
   public static class ElasticOut extends Easing.Elastic {
      ...
      public float ease(float t, float b, float c, float d) {
         ...
         if (t == 0.0F) {
            ...
            return a * (float)Math.pow(2.0, -10.0F * t) * (float)MathUtility.sin((t * d - s) * (Math.PI * 2) / p) + c + b;
         }
      }
   }
}

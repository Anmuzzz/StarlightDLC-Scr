package com.isusdlc.ui.menu.dropdown.components;
...
public class MenuPanel extends CustomComponent implements IScaledResolution {
   ...
   @Override
   public void onInit() {
      for (Module module : elegant.getInstance()
         .getModuleManager()
         .getModules()
         .stream()
   ...
   private void updateScale(UIContext context) {
      ...
      this.sizing
         .update(
            elegant.getInstance().getMenuScreen().isClosing()
               ? 2.0F
               : (
                  Math.abs(sr.getScaledWidth() / 2.0F - this.x) / 1500.0F * 3.0F < elegant.getInstance().getMenuScreen().getMenuAnimation().getValue()
                     ? 1.0F
                     : 0.0F
               )
         );
   ...
   public void renderBackground(UIContext context) {
      float alpha = elegant.getInstance().getMenuScreen().isClosing()
         ? elegant.getInstance().getMenuScreen().getMenuAnimation().getValue()
      ...
      context.drawRoundedRect(
         ...
         Colors.getBackgroundColor().withAlpha(255.0F * (elegant.getInstance().getThemeManager().getCurrentTheme() == Theme.DARK ? 0.6F : 0.75F))
      );
   ...
   public void renderShadow(UIContext context) {
      ...
      RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, alpha);
      this.scale(context);
      context.drawShadow(this.x, this.y, this.width, this.height, 20.0F, BorderRadius.all(8.0F), ColorRGBA.BLACK.withAlpha(64.0F));
      RenderUtility.end(context.getMatrices());
      RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
   }

   public void renderBlur(UIContext context) {
      float alpha = elegant.getInstance().getMenuScreen().isClosing() ? 2.0F - this.sizing.getValue() : this.sizing.getValue();
      this.scale(context);
   ...
   protected void renderComponent(UIContext context) {
      ...
      context.drawRoundedRect(
         ...
         this.height,
         BorderRadius.all(8.0F),
         Colors.getBackgroundColor().withAlpha(255.0F * (dark ? 0.92F : 0.8F))
      );
      context.drawRoundedRect(
         this.x,
         this.y,
         ...
         headerHeight,
         BorderRadius.all(8.0F),
         Colors.getBackgroundColor().mix(Colors.BLACK, dark ? 0.4F : 0.25F).withAlpha(255.0F * 0.98F)
      );

package com.isusdlc.ui.menu.dropdown.components.settings.impl;
   ...
   protected void renderComponent(UIContext context) {
      if (!this.initialized) {
         for (SelectSetting.Value value : this.setting.getValues()) {
            value.setEnablePenis(new PenisPlayer(elegant.id("penises/check_enable.penis")));
            value.setDisablePenis(new PenisPlayer(elegant.id("penises/check_disable.penis")));
      ...
      for (SelectSetting.Value valuex : this.setting.getValues()) {
         if (!valuex.isHidden()) {
            ...
            if (this.setting.isDraggable()) {
               context.drawTexture(elegant.id("icons/hud/drag.png"), x + 7.0F, y + valuex.getYAnim().getValue(), 6.0F, 6.0F, Colors.getTextColor());
            }

package com.isusdlc.systems.modules.impl;
...
public abstract class BaseModule implements Module {
   ...
   public void setEnabled(boolean newState, boolean silent) {
      if (this.enabled != newState) {
         ...
         if (!(this instanceof MenuModule) && elegant.getInstance().getModuleManager().getModule(Sounds.class).isEnabled() && !silent) {
            ClientSounds.MODULE
               .play(elegant.getInstance().getModuleManager().getModule(Sounds.class).getVolume().getCurrentValue(), this.enabled ? 1.1F : 1.0F);
         }
         ...
         if (this.enabled) {
            ...
            elegant.getInstance().getEventManager().unsubscribe(this);
            if (!silent) {
               elegant.getInstance()
                  .getNotificationManager()
                  .addNotification(

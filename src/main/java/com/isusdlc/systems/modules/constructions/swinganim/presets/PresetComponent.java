package com.isusdlc.systems.modules.constructions.swinganim.presets;
...
import com.isusdlc.systems.modules.constructions.swinganim.SwingPhase;
...
public class PresetComponent extends CustomComponent {
   ...
   private void create() {
      ...
      for (Setting setting : elegant.getInstance().getSwingManager().getStartPhase().getSettings()) {
         if (setting instanceof SwingPhase.PhaseSlider slider) {
            slider.setCurrentValue(0.0F);
         }
      }

      for (Setting settingx : elegant.getInstance().getSwingManager().getEndPhase().getSettings()) {
   ...
   public float getHeight() {
      ...
      return this.height = this.heightAnim
         .update(Math.min(presets.size() * 12 + elegant.getInstance().getSwingManager().getPresets().size() * 12 - 12, 182) + 46);
   }
}
